...
// This function converts the 1m dataframe to other time intervals
...
rearrange_candles(double** candles, string tf, long long from_time, long long to_time, int array_size)
{
    ...
    double tf_ms;
    // Check to see if it has to be converted to a differnt time interval:
    // m1 -> m5, m10, m15, m30, h1, h4, d1 and etc.
    ...
    // candles[0][0] -> First availabe record on dataframe sorted based on timestamp.
    // Round down the timestamp to requested resolution  by calculating the remainder
    // between the current_ts and the desired resolution in milliseconds using the
    // fmod function from the math library.
    ...
    {   
        ...
        if (candles[i][0] >= current_ts + tf_ms) {
            ...
            // Calculate the number of missing_candles
