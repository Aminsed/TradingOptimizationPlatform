...
void Psar::execute_backtest(double initial_acc, double acc_increment, double max_acc)
{
    ...
    int trend[2] = {0, 0}; // Up trend == positive && down trend == negative
    double sar[2] = {0.0, 0.0};
    double ep[2] = {0.0, 0.0}; // Expereme Point
    double af[2] = {0.0, 0.0}; // Acceleration Factor

    double temp_sar = 0.0;

    // Initialize values for starting point
    ...
    // We start from the third candle because we used the first two candles
    // to calculate initial values.
    for(int i = 2; i < ts.size(); i++)
    {
        // First step is to determine the trend
        ...
        // Then we calculate the new Extereme Point
        ...
        // Now we calculate the Acceleration Factor and SAR

        if (abs(trend[1]) == 1) { // If trend has just changed
            sar[1] = ep[0];
            af[1] = initial_acc;
        }
        else {
